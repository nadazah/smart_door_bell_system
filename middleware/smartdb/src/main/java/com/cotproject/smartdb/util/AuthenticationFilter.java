...
public class AuthenticationFilter implements ContainerRequestFilter {// AuthenticationFilter will block requests that do not contain JWT in their header
    ...
    public void filter(ContainerRequestContext containerRequestContext) throws IOException {
        ...
        final String path = containerRequestContext.getUriInfo().getRequestUri().getPath();
        if(path.equals(authorizePath)||path.contains(tokenPath)||path.equals(authenticatePath)||path.equals(personpath)||path.contains(forgottenpasswordpath)){
